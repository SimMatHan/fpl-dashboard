...
# Serve static files from the ../frontend folder (relative to this file)
...
def get_league(liga_id: int):
    """Liga standings with automatic pagination (all pages)."""
    try:
        page = 1
        all_results = []
        league_info = None
...
def get_history(manager_id: int):
    """GW-by-GW history + chip usage for a single manager."""
    try:
...
def get_picks(manager_id: int, gw: int):
    """Squad picks for a specific manager and gameweek."""
    try:
